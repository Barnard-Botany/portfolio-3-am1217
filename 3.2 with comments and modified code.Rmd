...
#This tree is very impressive becuase it provides an updated megaphylogeny of vascular plants, and it is also a tool that botanists can use the tree to gain more accurate infromation on the evolutionary relationships between plant species. It is both a huge source of knowledge based off of improved categorization AND a tool to explore evolutionary relationships.
...
#The zanne tree has longer branches that lead to tips, whereas the latter tree has shorter branches that in the zanne tree. The zanne tree is representing the notion that the species took longer to evolve over time than did the families.
...
```{r}
my_tree <- tol_induced_subtree(ott_ids = P_resolved_names$ott_id)
plot(my_tree, no.margin=TRUE)
```

2. You've developed your own lists of plants to resolve and phylogenize. Use it to Get a phylogeny from OTT.
```{r}
#making a phylogeny from OTT using our lists step 1. defining the scope of the tree
library(rotl)
taxa <- c("Prunus", "Betula", "Alliaria", "Bauhinia","Cornus", "Populus", "Ribes", "Salix", "Lavendula", "Duchesnea", "Acer", "Lonicera", "Syringa", "Sibbaldia", "Sorbus", "Cynanchum", "Spathiphyllum")
resolved_names <- tnrs_match_names(taxa, context_name = "Animals")
resolved_names

```
```{r}
#now we will make sure this is limited to land plants
library(rotl)
taxa <- c("Prunus", "Betula", "Alliaria", "Bauhinia","Cornus", "Populus", "Ribes", "Salix", "Lavendula", "Duchesnea", "Acer", "Lonicera", "Syringa", "Sibbaldia", "Sorbus", "Cynanchum", "Spathiphyllum")
resolved_names <- tnrs_match_names(taxa, context_name = "Land plants")
resolved_names
```
```{r}
my_tree <- tol_induced_subtree(ott_ids = resolved_names$ott_id)
plot(my_tree, no.margin=TRUE)
```
#here I made a phylogeny using my list of plants via the TOL data base! This portion of the portfolio has been successful in teaching me how to use powerful data reservoirs and trimming them down to the specific scope of evolutionary relationships that I am interested in, specifically the relationships between the plants that I was given in the article! 
