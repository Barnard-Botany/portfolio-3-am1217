...
b. Use commands such as "str" or "attribute" and comment about whether your erroneous and correct tree differ in tips, nodes, or edges. 
...
```{r}
#r20120829 is formally called an argument
...
```{r}
#here the stored stree is different but the taxa are the same these tree will look different because they run different codes. In zanne the tips are capitalized and they are in different order, also the branches are different lengths. dichotomous branching are present and polytomy forces a species into a group when the code isnt sure that the branch should go there. 
...
tree_phylomatic <- phylomatic(taxa = spp1, storedtree ="zanne2014", get = 'POST')
plot(tree_phylomatic, no.margin = TRUE)
#you see polytomy in the ABIES genus because they are difficult to tell apart
```
...
#caldogram is the one with Vshaped branching and the phylotgenetic tree is the vanne tree with differing branch lengths 
#all of the red message in the "R console" tells you everything that is not found within the zannetree
#zanne tree is simply more comprehensive, which is why we used it 
```
